...
- 版本以 `manifest.json.version`（Extension）與 `native/host.rb` 的 `HOST_VERSION`（Host）為單一真相；本檔不硬寫版號以免過時。發布／上架快照見 `AGENTS.md`
- 寫入方式：預設 Native Helper；Local REST API 為相容選項（27123 只有 HTTP、27124 才有 HTTPS 自簽憑證）
...
17. REST 協定：27124 走 HTTPS（自簽），其餘 port 走 HTTP。**不得強制把 27123 升成 HTTPS，也不得把預設埠改成 27124**——外掛只在 27124 提供 TLS，強改會斷線或踩自簽憑證；明文風險改以 popup 警告（`#httpWarning`）提醒。(v2.4.4 F3)
18. `onIntercept` 是 web→本機的信任邊界，且 MAIN world 的 `postMessage` **無 origin／token 檢查**：`pendingPost` 為 null 時只接受「備援已送出且仍在 THREAD_WINDOW 內」的遲到修正，其餘（串文後續、時窗外、頁面上任何腳本偽造的回應）一律忽略。**絕不對未對應發佈的攔截回應建檔**，否則等於讓任意頁面腳本靜默寫入 Vault。(v2.4.4 F1)
19. 引用貼文是**他人撰寫的不可信內容**，寫入筆記前必須中和 Markdown/HTML 注入（frontmatter 一律 `escapeYaml`；本文引用區塊的格式機制見 `AGENTS.md`）。(v2.4.4 F2)
...
**真實 E2E 快照與待驗收項目見 `AGENTS.md`；validator 與自動測試通過不等於通過真實發文驗收。**
